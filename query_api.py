...
This module contains all the code involving the Met Museum API (documentation
found at https://metmuseum.github.io/). Involves querying the API for data
on Van Gogh's paintings.
"""
...
def query_api_topics():
    ...
    paintings within the Met Museum database. Returns a dictionary where
    the keys are topics and the values are counts for the number of occurences
    for that topic. Due to the API limiting requests per second, this function
